...
export default function KasirPage() {
  ...
  // fix bug #5: konsisten sama ExpenseModal/pengeluaran/catering — cek dummy store juga,
  // jangan cuma cek koneksi. Sebelumnya kode ini selalu coba insert ke Supabase walau
  // toko masih dummy (store_id 'dummy-store-001' bukan uuid valid → gagal, boros request).
  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      if (isDummy || !navigator.onLine) throw new Error('skip-supabase')
      const supabase = createClient()
      const { error } = await supabase.from('sales').insert(records)
      if (error) throw error
    } catch {
      records.forEach((r) =>
        addPendingSync({ table: 'sales', action: 'insert', payload: { ...r, id: crypto.randomUUID() } })
      )
    }
    ...
    setPayMethod('cash')
    setSubmitting(false)
  }, [cart, currentStore, isDummy, customerName, payMethod, addPendingSync, toast, totalItems])
        ...
        <div className="kasir-right">
          ...
          <div style={{ padding:'20px 20px 14px', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
            <p style={{ margin:'0 0 12px', fontSize:11, fontWeight:700,
              textTransform:'uppercase', letterSpacing:'0.1em', color:'var(--text-muted)' }}>Pesanan</p>
          ...
          <div style={{ padding:'14px 20px 24px', borderTop:'1px solid var(--border)', flexShrink:0 }}>
            <p style={{ margin:'0 0 8px', fontSize:11, fontWeight:700,
              textTransform:'uppercase', letterSpacing:'0.08em', color:'var(--text-muted)' }}>Metode Pembayaran</p>
        ...
        return (
          <div key={product.id} style={{
            ...
            <div onClick={() => onAdd(product)} style={{
              ...
              cursor:'pointer', fontSize:44, WebkitTapHighlightColor:'transparent',
            }}>
              {product.photo_url ? (
            ...
            <div style={{ padding:'10px 12px 12px' }}>
              ...
              <p style={{ margin:'0 0 10px', fontSize:13, fontWeight:800,
                color:'var(--accent)', fontFamily:'Nunito, sans-serif' }}>{formatRp(product.price)}</p>
              {inCart ? (
